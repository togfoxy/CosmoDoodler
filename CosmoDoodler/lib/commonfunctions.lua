...
function loadTableFromFile(datfilename)
    ...
    -- output: table or nil
	-- eg mytable = loadTableFromFile("mydata.dat")
    local thistable = {}
...
function saveTableToFile(datfilename, table)
    ...
    local savefile = savedir .. datfilename
	print("Trying to save data to " .. savefile)
...
function multiple(n, size)
	-- input: number
	-- input: the nearest multiple. Defaults to 10
	-- output: the nearest multiple
	assert(n ~= nil)
    size = size or 10
